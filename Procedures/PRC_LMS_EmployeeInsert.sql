IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PRC_LMS_EmployeeInsert]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [PRC_LMS_EmployeeInsert] AS' 
END
GO

ALTER PROCEDURE [dbo].[PRC_LMS_EmployeeInsert]
(
	--@cnt_id		int,
	@cnt_ucc 		varchar(10),
	@cnt_salutation		int,
...
7.0		26-07-2024		Sanchita		V2.0.48		Employee branch mapping table not updated when Employee is added in FSM Demo Live. Refer: 0027651
***********************************************************************************************************************************/
begin
